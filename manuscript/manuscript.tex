...
%examine change over time in just the elwha (mike will ask about this)
%explain why some SD rivers might have a larger effect than RD (related to slope, groundwater, dams?)
%add discharge locations to the map?
%probably should include a table of data sources and station ids
% make sure figure captions are all formatted appropriately
% look at differences in elwha pre and post dam
...
An additional set of post-hoc regressions was performed using factor loadings on shared trends (\textbf{Z}; Eq.2) as dependent variables, with landscape predictors again as independent variables. Here, the goal was to %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

To facilitate this, we used a reduced version of the full model. 
...
\textbf{Figure 4} Relationships between watershed elevation and climatic effects on T\textsubscript{water} \textbf{(a-c)}, and between watershed features and factor loadings on shared trends \textbf{(d-e)}. Regression lines indicate slopes significant at $\alpha=0.1$.
\end{center}
