...
% single out elwha as mostly unaffected by climate
% explain that the effect has not changed over time (nor the variance)

% fig1: buffering effect is visible as damping followed by jump in RS SD
    SD RS colder throughout the year, less influenced by changes in air.
    % spring snowmelt (hydrograph peak) is the most significant item
    RD actually remains warmer than air throughout the spring, and matches it through July.
% fig2: snow/glac streams are buffered, overall, from temp changes due to air and precip.
