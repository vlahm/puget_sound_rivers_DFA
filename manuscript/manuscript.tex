...
\begin{tikzpicture}
    ...
    \draw (-4.1, -1.5) node[text width=9.5em] {\normalsize \textbf{Figure 2} Relationships between watershed elevation and climatic effects on T\textsubscript{water} \textbf{(a-c)}, and other watershed predictors and factor loadings on shared trends \textbf{(d-g)}. Regression lines indicate slopes significant at $\alpha=0.05$.};
\end{tikzpicture}
