...
# 7 - getting credible intervals for effect size over time with STAN ####

#setup
library(rstan) #install rstan using instructions here: http://mc-stan.org/interfaces/rstan
if(!require('devtools')) install.packages('devtools')
if(!require('statss')){
    library(devtools)
    install_github("eric-ward/safs-timeseries/statss")
}
library(statss)
...
#using air temperature as the covariate, water temp as the response
covs_z = covs_z[1,, drop=FALSE]
cov_and_seas <- rbind(ccgen('fixed_individual'), covs_z)
cov_and_seas <- designer(design='effect_byMonth_acrossTime', sections=5, focal_months=5:8)

#fit
dfa_stan = fit_dfa(y=dat_z, covar=cov_and_seas, num_trends=4,
                   zscore=FALSE, iter=2e5, varIndx=rep(1,nn),
                   chains=2)
dfa_stan = fit_dfa2(y=dat_z, covar=cov_and_seas, num_trends=4,
                   zscore=FALSE, iter=2e5, varIndx=rep(1,nn),
                   chains=2, cores=2)
#varIndx argument allows diagonal and unequal or (equalvarcov?, calls it "shared variances")
#unconstrained not available yet
rot = rotate_trends(mod_3) #does varimax rotation, also from statss package

#names = rotated Z matrix (for each draw), rotated trends (for each draw), mean Z across draws,
#mean trends across draws, lower 2.5% on trends, upper 97.5%
names(rot)
#you get the same results as with MARSS(trends_mean)

#evaluation
library(loo)
loo(extract_log_lik(dfa_stan))$looic
loo(extract_log_lik(dfa_stan2))$looic

#plotting
#(trend uncertainty - not usually shown, and so super-cool)
data("harborSealWA")
dfa_1 = fit_dfa(y = t(harborSealWA[,-1]), num_trends = 1)
pars = extract(dfa_1) #... lots not supplied here. see the slides for lecture 13

par = extract(dfa_stan2)
ls.str(par)
